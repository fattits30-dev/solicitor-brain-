...
citation_checks = Counter(
    "solicitor_brain_citation_checks_total", "Total number of citation checks", ["success"], registry=metrics_registry
)

hallucination_blocks = Counter(
    "solicitor_brain_hallucination_blocks_total", "Total number of blocked hallucinations", registry=metrics_registry
)

# Document metrics
sign_offs = Counter(
    "solicitor_brain_sign_offs_total", "Total number of document sign-offs", ["status"], registry=metrics_registry
)
...
gpu_temperature = Gauge(
    "solicitor_brain_gpu_temperature_celsius", "GPU temperature in Celsius", registry=metrics_registry
)

gpu_usage = Gauge("solicitor_brain_gpu_usage_percent", "GPU usage percentage", registry=metrics_registry)

# KPI metrics
auto_file_precision = Gauge(
    "solicitor_brain_auto_file_precision", "Auto-file precision percentage", registry=metrics_registry
)

email_case_match = Gauge(
    "solicitor_brain_email_case_match", "Email to case matching percentage", registry=metrics_registry
)

fact_check_precision = Gauge(
    "solicitor_brain_fact_check_precision", "Fact check precision percentage", registry=metrics_registry
)

# Health check counter
health_checks = Counter(
    "solicitor_brain_health_checks_total", "Total number of health checks", registry=metrics_registry
)
