...
@asynccontextmanager
async def async_safe_file_write(
...
@contextmanager
def locked_file(filepath: str | Path, mode: str = "r+", timeout: float = 10.0) -> Generator[TextIO, None, None]:
    """
...
@asynccontextmanager
async def file_lock(lockfile: str | Path, timeout: float = 30.0) -> AsyncGenerator[None, None]:
    """
...
@contextmanager
def atomic_write(filepath: str | Path, mode: str = "w", **kwargs: Any) -> Generator[TextIO, None, None]:
    """
...
async def document_processor(
    filepath: str | Path, output_dir: Path | None = None
