...
class DatabaseManager:
    ...
    async def __aenter__(self) -> "DatabaseManager":
        ...
        self.async_session_maker = sessionmaker(
            self.engine, class_=AsyncSession, expire_on_commit=False  # type: ignore[arg-type]
...
class ServiceOrchestrator:
    ...
    def __enter__(self):
        """Start all services"""
        for _svc_name, service in self.services.items():
            service.__enter__()
        return self
    ...
    def __exit__(
        ...
        for _svc_name, service in reversed(list(self.services.items())):
            service.__exit__(exc_type, exc_val, exc_tb)

    def is_healthy(self) -> bool:
        """Check if all services are running"""
        return all(not (service.process and service.process.poll() is not None) for _, service in self.services.items())
